...
/**
 * Decide the correct destination for an authenticated child:
 * → /child/dashboard  if they already have a linked parent
 * → /auth/child       otherwise (show the PIN code screen)
 */
async function resolveChildDestination(childId: string): Promise<string> {
  try {
    const res = await fetch(`${API_URL}/family/child/dashboard?child_id=${childId}`);
    if (res.ok) {
...
export default function Page() {
  ...
  // ── Auto-redirect for already-authenticated children ───────────────
  useEffect(() => {
    const childId = localStorage.getItem('child_id');
    if (childId) {
      resolveChildDestination(childId).then((dest) => router.push(dest));
    } else {
      setChecking(false);
    }
  }, [router]);
    ...
    try {
      if (isLoginMode) {
        ...
        if (data.success) {
          ...
          const dest = await resolveChildDestination(data.child_id);
          router.push(dest);
        } else {
        ...
        if (data.success) {
          ...
          storage.setItem('child_code', data.child_code);
          if (data.name) storage.setItem('child_name', data.name);
          // New accounts always go to auth/child (need to share code)
